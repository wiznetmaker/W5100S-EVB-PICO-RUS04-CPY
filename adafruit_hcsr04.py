...
class HCSR04:
    ...
    def deinit(self) -> None:
        ...
        #self._echo.deinit()
    """    
    def change(self,value: int, change_pin: Pin):
        if value == 1: #change to trigger mode
            change_pin.direction = Direction.OUTPUT
        
        elif value == 0: #change to echo mode
            change_pin.direction = Direction.INPUT
        
        else:
            raise SyntaxError("Input Error in changing mode")
    ...
    def _dist_two_wire(self) -> float:
        #if _USE_PULSEIO:
            #self._trig.deinit()  # Discard any previous pulse values
        ...
        if pulselen >= 65535:
            ...
            # OK no hardware pulse support, we'll just do it by hand!
            # hang out while the pin is low
        while not self._trig.value:
            if time.monotonic() - timestamp > self._timeout:
                self._trig.direction = Direction.OUTPUT
                raise RuntimeError("Timed out")
        timestamp = time.monotonic()
        # track how long pin is high
        while self._trig.value:
            if time.monotonic() - timestamp > self._timeout:
                self._trig.direction = Direction.OUTPUT
                raise RuntimeError("Timed out")
        pulselen = time.monotonic() - timestamp
